...
3. **Laundry Domain (Operational Support Domain)**: Owns Laundry Item Master, Laundry Service Master, Laundry Charge Master, Rate Snapshots, Garment Lines, operational service fulfillment, chargeability determination, and `LaundryChargeRaised` domain events (aligned with `docs/LAUNDRY_SPECIFICATION.md`).
...
| **Laundry** | Laundry Domain (Operational Support) | Confirmed Chargeable Service (`LaundryChargeRaised` / `LAUNDRY_SPECIFICATION` §69, §74) | `LaundryDiscoveryAdapter` | Laundry Charge Master & Rate Snapshot (Rate × Qty) | N/A (1:1 with Stay) | **Finance Domain** (via Laundry Event / Billing Engine Finance Adapter) | Evaluates service date in period, claims unbilled charge, dispatches to Finance without calculating laundry pricing. |
