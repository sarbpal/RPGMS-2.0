...
- **Attributes**: `id`, `garmentLineId`, `serviceId`, `serviceName`, `requestedQuantity`, `fulfilledQuantity`, `rateSnapshot`, `fulfillmentStatus` (`PENDING`, `FULFILLED`, `UNFULFILLED`, `REWORK`), `charges` (array of `LaundryChargeRecord`), `createdAt`, `updatedAt`.
- **Invariants**: Requested quantity cannot exceed the Garment Line's physical piece count. Multiple services on one Garment Line never increase the physical piece count. Service fulfillment and chargeability are independently tracked per service allocation.

### Laundry Charge Record
- **Purpose**: Child Entity owned by Service Allocation representing a discrete, immutable business charge tranche generated by the BR-L-012 chargeability engine.
- **Attributes**: `id`, `businessChargeId` (e.g. `LTX-2026-0001:GL-001:LSRV-001:BRK-01`), `transactionId`, `garmentLineId`, `serviceId`, `bracketIndex`, `quantity`, `unitRate`, `totalAmount`, `currency`, `calculatedAt`, `status` (`PENDING_POSTING` | `POSTED`), `financeBillId`, `postedAt`.
- **Invariants**: Commercial calculation and business identity fields are permanently immutable. Lifecycle fields track Finance posting progress.
